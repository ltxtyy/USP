...
TensorFlow.js:
    $ cd .. && git clone https://github.com/zldrobit/tfjs-yolov5-example.git && cd tfjs-yolov5_local-example
    $ npm install
    $ ln -s ../../yolov5_local/yolov5s_web_model public/yolov5s_web_model
    $ npm start
"""
...
from local_yolos.yolov5.models.yolo import Detect
...
def export_saved_model(model,
    ...
    try:
        ...
        from local_yolos.tf import TFDetect, TFModel
...
def export_tflite(keras_model, im, file, int8, data, nms, agnostic_nms, prefix=colorstr('TensorFlow Lite:')):
    ...
    try:
        ...
        if int8:
            from local_yolos.tf import representative_dataset_gen
...
@torch.no_grad()
def run(
        data=ROOT / 'data/coco128.yaml',  # 'bdd-dataset.yaml path'
    ...
    if any((saved_model, pb, tflite, edgetpu, tfjs)):
        ...
        assert not tflite or not tfjs, 'TFLite and TF.js local_yolos must be exported separately, please pass only one type.'
    ...
    if any(f):
        LOGGER.info(f'\nExport complete ({time.time() - t:.2f}s)'
                    ...
                    f"\nPyTorch Hub:     model = torch.hub.load('ultralytics/yolov5_local', 'custom', '{f[-1]}')"
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/coco128.yaml', help='bdd-dataset.yaml path')
