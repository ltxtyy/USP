...
LOGGER = logging.getLogger("yolov5_local")  # define globally (used in train.py, val.py, detect.py, etc.)
...
def check_file(file, suffix=''):
    ...
    if Path(file).is_file() or not file:  # exists
        ...
        files = []
        for d in 'data', 'local_yolos', 'utils':  # search directories
...
def check_dataset(data, autodownload=True):
    # Download, check and/or unzip bdd-dataset if not found locally
